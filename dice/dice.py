...
def _mergeDiceDicts(d1, d2):
    ...
    """
    assert type(d1) == dict and type(d2) == dict, "Invalid argument to mergeDiceDicts!"
    if len(d1) == 0:
        return d2
    if len(d2) == 0:
        return d1
...
def diceDict(diceList):
    ...
    have repetitions).   
    """
    assert type(diceList) == list, "Invalid argument to diceDict!"
    if len(diceList) == 0:
        return dict()
    elif len(diceList) == 1:
...
def diceProb(diceList):
    ...
    have repetitions).       
    """
    assert type(diceList) == list, "Invalid argument to diceProb!"
...
def roll(diceList):
    """ 
    Returns a "roll" of the dice described in diceList.  
    ...
    have repetitions).
    """ 
    assert type(diceList) == list, "Invalid argument to roll!"
    result = 0
